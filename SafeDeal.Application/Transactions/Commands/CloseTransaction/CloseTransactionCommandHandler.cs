...
public class CloseTransactionCommandHandler : IRequestHandler<CloseTransactionCommand, TransactionDto>
{
    ...
    public CloseTransactionCommandHandler(ITransactionRepository transactions, IUserRepository users)
    {
    ...
    public async Task<TransactionDto> Handle(CloseTransactionCommand request, CancellationToken ct)
    {
        ...
        if (transaction.VendorId != request.VendorId)
            throw new ForbiddenException("Only the vendor can close this transaction.");
